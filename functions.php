...
/**
 * ボタンブロックにアイコンを追加する機能を実装
 */
function add_button_icon_feature() {
	wp_enqueue_script(
		'button-icon-feature',
		get_stylesheet_directory_uri() . '/assets/js/button-icon-feature.js',
		array( 
			'wp-blocks', 
			'wp-block-editor', 
			'wp-element', 
...
add_action( 'enqueue_block_editor_assets', 'add_button_icon_feature' );

/**
 * カラムブロックの方向を反転するトグル機能を追加
 */
function add_column_direction_toggle() {
	wp_enqueue_script(
		'column-direction-toggle',
		get_stylesheet_directory_uri() . '/assets/js/column-direction-toggle.js',
		array( 
			'wp-blocks', 
			'wp-block-editor', 
			'wp-element', 
...
add_action( 'enqueue_block_editor_assets', 'add_column_direction_toggle' );